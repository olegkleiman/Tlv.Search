...
internal class OpenAiService
{
    ...
    public async Task<string> ProcessUserInput(string input, Message[] history = null)
    {
        ...
        var intent = await DetermineIntent(input);
    ...
    public async Task<string> FrameQuestion(List<string> docTexts)
    {
        var summaries = await Task.WhenAll(docTexts.Select(text => SummarizeWithOpenAI(text)));
        var combinedSummary = string.Join(" ", summaries);
        var framedQuestion = $"Based on the following information: {combinedSummary} What insights can we draw about [User's Specific Query]?";
